...
constexpr int kMinSliderHeight = 10;
...
QRect CustomScrollBar::topButtonRect() const {
  return QRect(0, 0, width(), indicatorHeight());
}

QRect CustomScrollBar::pageUpButtonRect() const {
  return QRect(0, indicatorHeight(), width(), indicatorHeight());
}

QRect CustomScrollBar::pageDownButtonRect() const {
  return QRect(0, height() - indicatorHeight() * 2, width(), indicatorHeight());
}

QRect CustomScrollBar::bottomButtonRect() const {
  return QRect(0, height() - indicatorHeight(), width(), indicatorHeight());
}

// 计算轨道区域（去除了按钮占用的空间）
QRect CustomScrollBar::grooveRect() const {
  return QRect(0, 2 * indicatorHeight(), width(),
               height() - 4 * indicatorHeight());
}
int CustomScrollBar::indicatorHeight() const { return width() * 1.5; }
...
QRect CustomScrollBar::sliderRect() const {
  ...
  // 计算滑块高度
  double pageRatio =
      static_cast<double>(pageStep()) / (maximum() - minimum() + pageStep());
  int sliderHeight = qMax(kMinSliderHeight, static_cast<int>(groove.height() * pageRatio));
...
void CustomScrollBar::paintEvent(QPaintEvent *event) {
  ...
  draw3DButton(p, pageDownButtonRect(), m_turnDownRenderer,
               m_pageDownButtonPressed);
  draw3DButton(p, bottomButtonRect(), m_turnBottomRenderer,
               m_bottomButtonPressed);
}

void CustomScrollBar::draw3DButton(QPainter &p, const QRect &rect,
                                   QSvgRenderer *renderer, bool pressed) {
  // 绘制3D效果的按钮
...
void CustomScrollBar::draw3DButton(QPainter &p, const QRect &rect,
                                   const QString &text, bool pressed) {
  // 绘制3D效果的按钮
  QColor buttonColor = palette().button().color();
...
void CustomScrollBar::mousePressEvent(QMouseEvent *event) {
  ...
  if (groove.contains(event->pos())) {
    // 点击了轨道，跳转到相应位置
    if ((groove.height() - slider.height()) <= 0) return;
